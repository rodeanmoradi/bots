...
One function, call it per frame:
    ...
    pts = arm_xy(frame)          # (3, 2) array, or None if no one is tracked
    # pts[0] = shoulder (x, y)
    # pts[1] = elbow    (x, y)
    # pts[2] = hand     (x, y)
...
To build a trajectory, keep the frames you get back and stack them:
    ...
    while capturing:
        ...
        if pts is not None:
            traj.append(pts)
    traj = np.stack(traj)        # (n_frames, 3, 2)
...
def arm_xy(frame, side="right", plane="side", with_claw=False,
           min_visibility=0.5):
    """Shoulder, elbow and hand as a (3, 2) array. None if not tracked.
...
def open_camera(spec="iriun", width=1280, height=720):
    """Open a webcam by index, or by part of its name such as "iriun"."""
    index = None
    if str(spec).isdigit():
        index = int(spec)
    else:
        ...
        hits = [i for i, n in enumerate(names) if str(spec).lower() in n.lower()]
        if not hits:
    ...
    if not ok:
        cap.release()
        raise SystemExit(f"camera {index} gave no frame - is Iriun connected?")
    return cap


if __name__ == "__main__":
    # Demo: print the three points, and show them, until Esc.
    cap = open_camera("iriun")
    try:
        while True:
            ok, frame = cap.read()
            if not ok:
            ...
            if pts is not None:
                print(" ".join(f"{n}({x:+.2f},{y:+.2f})"
                               for n, (x, y) in zip(("sh", "el", "hand"), pts)),
                      end="\r")
